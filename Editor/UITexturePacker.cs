...
namespace GreatClock.Common.UGUITools {
	...
	public class UITexturePacker {
		...
		static public Rect[] PackTextures(Texture2D texture, Texture2D[] textures, int width, int height, int padding, int maxSize, bool forceSquare, out float scale) {
			return PackTexturesInternal(texture, textures, width, height, padding, maxSize, forceSquare, true, out scale);
		}

		static Rect[] PackTexturesInternal(Texture2D texture, Texture2D[] textures, int width, int height, int padding, int maxSize, bool forceSquare, bool checkScale, out float scale) {
			scale = 1f;
			...
			if (width > maxSize || height > maxSize) {
				...
				Texture2D[] texes = new Texture2D[textures.Length];
				float f0 = 0f;
				float f1 = 1f;
				float factor = 0f;
				for (int i = 0; i < 10; i++) {
					float f = (f0 + f1) * 0.5f;
					for (int j = 0, jmax = textures.Length; j < jmax; j++) {
						texes[j] = ResizeTexture(textures[j], f);
					}
					Rect[] trs = PackTexturesInternal(texture, texes, maxSize, maxSize, padding, maxSize, forceSquare, false, out scale);
					for (int j = 0, jmax = texes.Length; j < jmax; j++) {
						Object.DestroyImmediate(texes[j]);
						texes[j] = null;
					}
					if (trs == null) {
						f1 = f;
				...
				for (int i = 0, imax = textures.Length; i < imax; i++) {
					texes[i] = ResizeTexture(textures[i], factor);
				}
				Rect[] rs = PackTexturesInternal(texture, texes, maxSize, maxSize, padding, maxSize, forceSquare, false, out scale);
				for (int i = 0, imax = texes.Length; i < imax; i++) {
					Object.DestroyImmediate(texes[i]);
				}
				scale = factor;
				return rs;
			}
				...
				for (xPadding = 1; xPadding >= 0; --xPadding) {
					for (yPadding = 1; yPadding >= 0; --yPadding) {
						...
						// After having no padding if it still doesn't fit -- increase texture size.
							...
							return PackTexturesInternal(texture, textures, width, height,
								padding, maxSize, forceSquare, checkScale, out scale);
						}
					}
					if (rect.width != 0 && rect.height != 0) break;
				}
		...
		static Texture2D ResizeTexture(Texture2D tex, float factor) {
			if (tex == null || factor <= 0f) { return null; }
			int w = Mathf.RoundToInt(tex.width * factor);
			int h = Mathf.RoundToInt(tex.height * factor);
			if (w < 1) { w = 1; }
			if (h < 1) { h = 1; }
			RenderTexture rt = RenderTexture.GetTemporary(w, h, 0, RenderTextureFormat.ARGB32, RenderTextureReadWrite.sRGB, 8);
